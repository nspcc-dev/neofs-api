...
Result is ordered by the first attribute used in filters and object ID.
...
| version | [uint32](#uint32) |  | Version of the Query Language used. |
| filters | [SearchFilter](#neo.fs.v2.object.SearchFilter) | repeated | List of search expressions. Limited to 8. |
...
| attributes | [string](#string) | repeated | List of attribute names to include into the reply. Limited to 4, these attributes also affect result ordering. |
...
| cursor | [string](#string) |  | Cursor that can be used for subsequent requests, an opaque string that is omitted or empty when there are no more results to list. |
