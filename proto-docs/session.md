...
* __NEOFS__CONTAINER_VERSION \
  Starting from API v2.27.0 and for a limited set of requests only, requester
  can attach container version to extended headers to ensure container state
  is up to date. If server's known version for container is higher, it must
  return **OUTDATED_CONTAINER** (3076) response status with no payload.
